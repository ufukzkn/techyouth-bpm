...
- SuperAdmin user detail includes a two-step community-transfer panel. Preview loads only target-community roles, shows before/after context and blocks movement while direct/claimed active tasks exist. Confirmed transfer closes sessions and preserves historical process actors.
- Team roster rows expose active workload counts. A member sees their own details; team leads, `Teams.Manage` holders and SuperAdmin may inspect another member's server-paged active work.
- Opaque-session resolution uses a 15-second in-process cache by default. Logout, profile/password, access, community lifecycle/role and team-membership changes invalidate affected entries immediately; this removes repeated permission-graph SQL without weakening the application-service authorization boundary.
- Next.js 16 Turbopack already keeps its development filesystem cache enabled. First visits in `next dev` may still compile lazy route chunks; Docker and `npm run build && npm start` use precompiled production chunks and are the correct navigation-performance baseline.
