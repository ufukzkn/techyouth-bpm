...
- Latest verified backend baseline is 103 passing tests. Service tests use relational SQLite instead of EF InMemory. `WebApplicationFactory` tests cover real HTTP cookies, CSRF, Bearer/Swagger compatibility, refresh rotation/reuse, logout, `429` rate limiting, inactive-community login, management scope and form-to-process smoke behavior.
