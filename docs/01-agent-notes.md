...
- Workspace routing standard: protected routes own their `WorkspaceShell` wrapper and pass only the required shell context to the route view. Keep the shell route-aware and lightweight; do not introduce route-wide prefetching that makes development navigation feel blocked.
...
- Chart and detail disclosure rule: dashboard segment hover may show tooltip/center values but must never change SVG geometry. User-detail chronological history stays collapsed until explicitly opened through the disclosure control.
