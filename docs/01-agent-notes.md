...
- AppShell was modularized after the access/log UX work. Keep `AppShell.tsx` focused on session, route, navigation and active-view composition; put dashboard/settings/users/logs UI in `src/features/app-shell/views`, shared shell UI in `components`, audit helpers in `auditUtils` and session/date/user formatters in `sessionFormatters`.
