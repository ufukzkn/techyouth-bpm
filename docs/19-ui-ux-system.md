...
- Form Designer keeps its heading, form/version selector and version actions in the initial route shell. The DnD canvas, palette and mobile palette are loaded as a client-only chunk with a fixed regional skeleton; do not move the shell behind that dynamic boundary. The first `/forms` compilation in `next dev` remains a development-only Turbopack cost and is not representative of the production route transition.
